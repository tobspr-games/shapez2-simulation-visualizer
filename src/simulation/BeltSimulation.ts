...
export class BeltSimulation {
  protected UpdateLane(lane: BeltLane, delta_T: int): bool {
    if (delta_T < 0n) {
  ...
  protected UpdateLaneMaxProgressWithItem(lane: BeltLane): void {
    var steps_S: int = lane.Progress_T * lane.Definition.StepsPerTick_S;
    lane.MaxStep_S = min(0n, steps_S - BeltLaneDefinition.SPACING_S);
  }

  // Public so the belt path can access it
  public TransferToLane(itemToTransfer: BeltItem, nextLane: BeltLane, remainingTicks: int): bool {
    if (nextLane.HasItem) {
      return false;
    }
